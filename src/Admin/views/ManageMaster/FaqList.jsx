...
import { faqListAction } from 'Admin/actions/faq';
import { faqchangestatusAction } from 'Admin/actions/faq';
...
class FaqList extends Component {
    ...
    editButton(cell, row, enumObject, rowIndex) {
        return (<p><Link to={{ pathname: `update-faq/`, state: { row } }} onClick={this.toggleEdit.bind(this, row)} ><i class="fa fa-pencil" aria-hidden="true"></i></Link>
            &nbsp;&nbsp; &nbsp;&nbsp;
    ...
    navigateto() {
        window.location.href = "Admin/add-faq";
        this.clicked = "";
    }
    componentDidMount() {
        this.props.faqListAction(this.state)
    }
    ...
    getdata_answer(cell, row, enumobject, rowIndex, ref) {
        let data = '';
        data = enumobject.answer.english;
        return data;
    }
                            ...
                            <Card
                                content={
                                    <div className="fresh-datatables">
                                        <Button bsStyle="info" fill pullRight onClick={() => this.props.history.replace('add-faq')}>Add Faq</Button>
                                        <BootstrapTable selectRow="{selectRowProp}" deleteRow={false} data={this.state.faqList} search={true} multiColumnSearch={true} pagination={true} options={options} striped hover condensed scrollTop={'Bottom'}>
                                            ...
                                            <TableHeaderColumn thStyle={{ width: '20%' }} tdStyle={{ width: '20%' }} tdAttr={{ 'data-attr': 'Question English' }} dataField='question' dataSort={true} dataFormat={this.imploadArray.bind(this, "question")}>English Question </TableHeaderColumn>
                                            <TableHeaderColumn thStyle={{ width: '20%' }} tdStyle={{ width: '20%' }} tdAttr={{ 'data-attr': 'Answer English' }} dataField='answer' dataSort={true} dataFormat={this.getdata_answer.bind(this, "answer")}>English Answer </TableHeaderColumn>
...
export default withRouter(connect(mapStateToProps, { faqListAction,faqchangestatusAction })(FaqList));
